#Important Note - Please download the 2R_planar_robot URDF file which we have provided in the google drive folder and run our simulation using that as there are some changes in the URDF file
                  #which we made in order to facilitate faster simulation
...
file_name = "2R_planar_robot.urdf"   #the URDF file we have loaded is different from the one that was given. We have changed the velocities of the joints in the URDF to facilitate faster simulation
p.connect(p.GUI)
...
point_A = [0,0,2.1]    #endpoints of the line that our robot's end-effector will trace during this simulation
point_B = [0,2.1,0]
...
def Inverse_kinematics(target,num):   #num has been included as different inverse kinematics are required for when the end effector is above z=1 and when it is below z=1
	'''
	This function should do the necessary trignometric 
	...
	if b==0:  #included to avoid float division by zero
		phi=np.pi/2
	else:
		phi=math.atan(a/b)
...
'''
count=0  #to keep track as to whether the end effector is going up or down
angle_1=0
angle_2=0
ee_coord=[0,0,2]
num=0    #to keep track as to whether the end effector is below z=1 or above z=1
...
while(True):
	...
	if ee_coord[2]<0.01:
		count=1
	elif ee_coord[2]>1.99:
		count=0

	ee_coord[1]=2-ee_coord[2]
	...
	angle_1,angle_2  = Inverse_kinematics(ee_coord,num)
